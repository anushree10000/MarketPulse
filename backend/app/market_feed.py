...
def seed_symbol_from_quote(db: Session, sym: "models.Symbol", quote: dict, now: datetime):
    ...
    sym.sma_short = price
    sym.sma_long = price
    sym.last_volume = sym.volume_mean  # no volume in a lightweight quote endpoint; start at the model default
    sym.source = "live_quote"
    sym.updated_at = now
