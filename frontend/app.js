...
function chartThemeColors() {
  ...
  return {
    line: styles.getPropertyValue("--accent").trim(),
    grid: styles.getPropertyValue("--border").trim(),
    text: styles.getPropertyValue("--text-tertiary").trim(),
  };
}
...
async function loadPriceChart(symbol) {
  ...
  if (typeof Chart === "undefined") {
    // Chart.js failed to load (e.g. the CDN is unreachable) -- degrade
    // quietly rather than breaking the rest of the detail view, which
  ...
  detailChart = new Chart(canvas, {
    ...
    options: {
      ...
      scales: {
        x: {
          grid: { display: false },
          ticks: { color: colors.text, maxTicksLimit: 5, autoSkip: true, font: { size: 10 } },
        },
        y: {
